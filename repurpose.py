#!/usr/bin/env python3
"""
Product with Attitude — Full Repurposer Pipeline
==================================================
One script. Zero AI credits. All mechanical steps automated.
...
def _qiita_tags_from_article(title, description):
    """Generate Qiita tags (max 5). Qiita tags are case-sensitive, no spaces."""
    combined = f"{title} {description}".lower()
    tag_map = {
        "AI": True,  # Always include
        "Claude": ["claude", "anthropic", "cowork"],
        "Perplexity": ["perplexity"],
        "ChatGPT": ["chatgpt", "openai"],
        "Replit": ["replit"],
        "ProductManagement": ["product manag", "product think", "product strateg"],
        "vibecoding": ["vibecod", "vibe cod", "vibe-cod"],
        "MachineLearning": ["machine learn", "deep learn", "llm"],
        "Substack": ["substack"],
        "automation": ["workflow", "automat"],
        "programming": ["coding", "code", "developer"],
    }
    tags = ["AI"]  # Always first
    for tag, patterns in tag_map.items():
        if tag == "AI":
            continue
        if isinstance(patterns, list) and any(p in combined for p in patterns):
            tags.append(tag)
    return tags[:5]


def _get_existing_qiita_titles(qiita_token):
    """Fetch existing article titles from Qiita to avoid duplicates."""
    existing = set()
    page = 1
    while True:
        req = urllib.request.Request(
            f"{QIITA_API_URL}/authenticated_user/items?page={page}&per_page=100",
            headers={
                "Authorization": f"Bearer {qiita_token}",
                "User-Agent": "PWA-Repurposer/1.0",
            },
        )
        try:
            with urllib.request.urlopen(req, timeout=30) as resp:
...
def create_qiita_body(article, gist_content):
    """Create Qiita article body: gist content WITHOUT the For Machines section."""
    if not gist_content:
        gist_content = create_gist_content(article)

    # Remove the For Machines section (everything from "### For Machines" onward)
    fm_idx = gist_content.find("### For Machines")
    if fm_idx > 0:
        # Also remove the --- separator before it
        pre = gist_content[:fm_idx].rstrip()
        if pre.endswith("---"):
            pre = pre[:-3].rstrip()
        body = pre
    else:
    ...
    # Ensure it ends with the CTA link
    if "Read on Substack" not in body and "Visit on Substack" not in body:
        body += f"\n\n---\n\n**[Read the full post on Substack →]({article['link']})**"

    return body.strip()


def publish_to_qiita(qiita_token, article, body_markdown):
    """Publish an article to Qiita and return the URL."""
    tags = _qiita_tags_from_article(article["title"], article.get("description", ""))
    ...
    try:
        ...
        error_body = e.read().decode("utf-8", errors="replace")
        print(f"      ERROR posting to Qiita ({e.code}): {error_body[:200]}")
        return None
    except Exception as e:
        print(f"      ERROR posting to Qiita: {e}")
        return None
...
BSKY_HANDLE = "productsattitude.bsky.social"


def _bsky_create_session(password):
    """Authenticate with Bluesky and return session (accessJwt + did)."""
    payload = json.dumps({
...
def _bsky_generate_hashtags(title, description):
    """Generate 3-5 CamelCase hashtags for Bluesky."""
    combined = f"{title} {description}".lower()
    tag_map = {
        "#VibeCoding": ["vibecod", "vibe cod"],
        "#AITools": ["ai tool", "tool review"],
        "#AIProduct": ["product manag", "product think"],
        "#Claude": ["claude", "anthropic"],
        "#Perplexity": ["perplexity"],
        "#AIAgents": ["ai agent", "agentic"],
        "#BuildInPublic": ["build in public"],
        "#AILiteracy": ["ai literacy"],
        "#Substack": ["substack"],
        "#OpenSource": ["open source", "open-source"],
        "#LLM": ["llm", "large language model"],
        "#PromptEngineering": ["prompt engineer"],
    }
    tags = []
    for tag, patterns in tag_map.items():
        if any(p in combined for p in patterns):
    ...
    if not tags:
        tags = ["#AI"]
    tags.append("#ProductWithAttitude")
    return tags[:5]


def _bsky_build_facets(text):
    """Build Bluesky facets for hashtags and URLs in the post text."""
    facets = []
...
def _bsky_create_one_sentence_description(title, description):
    """Create a concise 1-sentence description from the article metadata."""
...
def post_to_bluesky(password, article):
    """Post a 1-sentence description + link + hashtags to Bluesky. Returns post URL."""
    try:
        session = _bsky_create_session(password)
    except Exception as e:
    ...
    desc = _bsky_create_one_sentence_description(
        article["title"], article.get("description", "")
    )
    hashtags = _bsky_generate_hashtags(article["title"], article.get("description", ""))
    link = article["link"]

    text = f"{desc}\n\n{link}\n\n{' '.join(hashtags)}"

    # Truncate if needed (Bluesky has a 300-grapheme limit)
    if len(text) > 295:
        max_desc = 295 - len(f"\n\n{link}\n\n{' '.join(hashtags)}")
        desc = desc[:max_desc - 3] + "..."
        text = f"{desc}\n\n{link}\n\n{' '.join(hashtags)}"

    facets = _bsky_build_facets(text)
    ...
    try:
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            return f"https://bsky.app/profile/{BSKY_HANDLE}/post/{rkey}" if rkey else uri
    except urllib.error.HTTPError as e:
        error_body = e.read().decode("utf-8", errors="replace")
        print(f"      ERROR posting to Bluesky ({e.code}): {error_body[:200]}")
        return None
    except Exception as e:
...
def check_intelligence_flag(article):
    """Check if an article mentions Claude or Perplexity and should be flagged
    for manual hub page review. Returns a list of matched topics or empty list."""
    combined = f"{article['title']} {article.get('description', '')} {article.get('plainText', '')[:2000]}".lower()
    matched = set()
    for keyword, topic in INTELLIGENCE_KEYWORDS.items():
...
def main():
    parser = argparse.ArgumentParser(
        description="Product with Attitude — Full Repurposer Pipeline"
    )
    # Tokens / API keys
    parser.add_argument("--token", default=os.environ.get("GITHUB_TOKEN"),
                        help="GitHub fine-grained token for repo push (or GITHUB_TOKEN env)")
    parser.add_argument("--gist-token", default=os.environ.get("GITHUB_GIST_TOKEN"),
                        help="GitHub classic token for gist creation (or GITHUB_GIST_TOKEN env). Falls back to --token.")
    parser.add_argument("--devto-key", default=os.environ.get("DEVTO_API_KEY"),
                        help="Dev.to API key (or DEVTO_API_KEY env)")
    parser.add_argument("--qiita-token", default=os.environ.get("QIITA_TOKEN"),
                        help="Qiita access token (or QIITA_TOKEN env)")
    parser.add_argument("--bluesky-password", default=os.environ.get("BLUESKY_APP_PASSWORD"),
                        help="Bluesky app password (or BLUESKY_APP_PASSWORD env)")
    # Toggles
    parser.add_argument("--dry-run", action="store_true",
                        help="Show what would change without writing/publishing anything")
    ...
    parser.add_argument("--no-qiita", action="store_true",
    ...
    qiita_token = getattr(args, 'qiita_token', None)
    ...
    if args.no_push or args.dry_run:
        repo_dir = "."
        print(f"[0/{TOTAL_STEPS}] Using local files")
    else:
        repo_dir = tempfile.mkdtemp(prefix="pwa_schema_")
        print(f"[0/{TOTAL_STEPS}] Cloning repo...")
    ...
    # ── Step 1: Fetch RSS ───────────────────────────────────────────────
    print(f"[1/{TOTAL_STEPS}] Fetching RSS feed...")
    try:
    ...
    # ── Step 2: Load schema ─────────────────────────────────────────────
    print(f"[2/{TOTAL_STEPS}] Loading schema...")
    try:
    ...
    # ── Step 3: Detect new articles ─────────────────────────────────────
    print(f"[3/{TOTAL_STEPS}] Detecting new articles...")
    ...
    # ── Step 4: Update for_machines.json ────────────────────────────────
    print(f"[4/{TOTAL_STEPS}] Updating for_machines.json...")
    ...
    # ── Step 5: Update canonical links ──────────────────────────────────
    print(f"[5/{TOTAL_STEPS}] Updating canonical links...")
    try:
    ...
    print(f"[6/{TOTAL_STEPS}] Updating llms.txt...")
    ...
    # ── Step 7: Create & publish gists (free articles only) ─────────────
    print(f"[7/{TOTAL_STEPS}] Creating GitHub gists...")
    gist_urls = {}
    gist_contents = {}  # title -> markdown content, reused for Qiita
    for article in new_articles:
    ...
    # ── Step 8: Cross-post to dev.to (free articles only) ──────────────
    print(f"[8/{TOTAL_STEPS}] Cross-posting to dev.to...")
    devto_urls = {}
    if args.no_devto:
        print("      [SKIPPED] --no-devto flag set")
    elif not devto_key:
        print("      [SKIPPED] No dev.to API key provided")
    else:
    ...
    # ── Step 9: Cross-post to Qiita (free articles only) ───────────────
    print(f"[9/{TOTAL_STEPS}] Cross-posting to Qiita...")
    qiita_urls = {}
    if args.no_qiita:
        print("      [SKIPPED] --no-qiita flag set")
    elif not qiita_token:
        print("      [SKIPPED] No Qiita token provided (use --qiita-token or QIITA_TOKEN)")
    else:
        print("      Checking for existing articles on Qiita...")
        existing_qiita = _get_existing_qiita_titles(qiita_token)
        print(f"      Found {len(existing_qiita)} existing articles on Qiita")

        qiita_publish_count = 0
        for article in new_articles:
            if not article["isAccessibleForFree"]:
                print(f"      SKIPPED (paid): {article['title']}")
                continue
            if _normalize_title(article["title"]) in existing_qiita:
                print(f"      SKIPPED (already on Qiita): {article['title']}")
                continue

            gist_md = gist_contents.get(article["title"])
            qiita_body = create_qiita_body(article, gist_md)

            if args.dry_run:
                tags = _qiita_tags_from_article(article["title"], article.get("description", ""))
                print(f"\n      [DRY RUN] Qiita for: {article['title']}")
                print(f"        tags: {tags}")
                print(f"        body: {len(qiita_body)} chars")
            else:
                if qiita_publish_count > 0:
                    import time as _time
                    _time.sleep(5)  # Qiita rate limit
                url = publish_to_qiita(qiita_token, article, qiita_body)
                if url:
                    qiita_urls[article["title"]] = url
                    qiita_publish_count += 1
                    print(f"      Published: {article['title']}")
                    print(f"        -> {url}")
                else:
                    print(f"      FAILED: {article['title']}")

    # ── Step 10: Post to Bluesky (free articles only) ──────────────────
    print(f"[10/{TOTAL_STEPS}] Posting to Bluesky...")
    bluesky_urls = {}
    if args.no_bluesky:
        print("      [SKIPPED] --no-bluesky flag set")
    elif not bluesky_pw:
        print("      [SKIPPED] No Bluesky password provided (use --bluesky-password or BLUESKY_APP_PASSWORD)")
    else:
        for article in new_articles:
            if not article["isAccessibleForFree"]:
                print(f"      SKIPPED (paid): {article['title']}")
            ...
            if args.dry_run:
                desc = _bsky_create_one_sentence_description(article["title"], article.get("description", ""))
                hashtags = _bsky_generate_hashtags(article["title"], article.get("description", ""))
                print(f"\n      [DRY RUN] Bluesky for: {article['title']}")
                print(f"        desc: {desc}")
                print(f"        hashtags: {' '.join(hashtags)}")
            else:
                url = post_to_bluesky(bluesky_pw, article)
                if url:
                    bluesky_urls[article["title"]] = url
    ...
    intel_flags = {}
    for article in new_articles:
        flags = check_intelligence_flag(article)
        if flags:
            intel_flags[article["title"]] = flags

    if intel_flags:
        print()
        print("      ╔══════════════════════════════════════════════════════╗")
        print("      ║  INTELLIGENCE FLAG — Manual hub review needed       ║")
        print("      ╚══════════════════════════════════════════════════════╝")
        for title, topics in intel_flags.items():
            print(f"      → {title}")
            print(f"        Topics: {', '.join(topics)}")
            print(f"        Action: Review hub page formatting & suggest placement")
        print()
    else:
        print("      No Claude/Perplexity content detected — no hub review needed.")

    # Write files & push
    if args.dry_run:
        print(f"\n{'=' * 65}")
        print("  DRY RUN — No files modified, nothing pushed, nothing posted")
        print("=" * 65)
    else:
